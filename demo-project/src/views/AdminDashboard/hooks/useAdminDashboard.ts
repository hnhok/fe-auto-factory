/**
 * useAdminDashboard — 后台控制面板 页面 Composable
...
import { ref, reactive, onMounted } from 'vue'
import { showToast } from 'vant'
import { getSystemStats } from '@/api/admin-dashboard'

export function useAdminDashboard() {
  // ─── 状态定义 ────────────────────────────────────────
  const loading = ref(false)
  const error = ref<string | null>(null)
  const adminDashboardData = reactive<Record<string, any>>({
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      Object.assign(adminDashboardData, res?.data ?? res)
    } catch (e: any) {
