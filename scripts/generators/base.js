...
export function generateMockFile({ cwd, page_id, kebab, models }) {
  ...
  /**
   * Smart Mock Helper: 根据字段名推导更合理的 Mock 数据
   */
  const getSmartMockValue = (name, type) => {
    const lowerName = name.toLowerCase()
    if (type === 'string') {
      if (lowerName.includes('avatar') || lowerName.includes('image') || lowerName.includes('img')) return `@image('200x200', '@color', '#FFF', 'img')`
      if (lowerName.includes('email') || lowerName.includes('mail')) return `@email`
      if (lowerName.includes('name') || lowerName.includes('user')) return `@cname`
      if (lowerName.includes('phone') || lowerName.includes('tel')) return /^1[3-9]\d{9}$/ // 生成正则 Mock
      if (lowerName.includes('time') || lowerName.includes('date')) return `@datetime`
      if (lowerName.includes('price') || lowerName.includes('amount')) return `@float(1, 1000, 2, 2)`
      if (lowerName.includes('desc') || lowerName.includes('content') || lowerName.includes('title')) return `@ctitle(10, 20)`
      return `@ctitle(5)`
    }
    if (type === 'number') {
      if (lowerName.includes('price') || lowerName.includes('amount')) return `@float(1, 1000, 2, 2)`
      if (lowerName.includes('count') || lowerName.includes('num') || lowerName.includes('stock')) return `@integer(0, 500)`
      if (lowerName.includes('id')) return `@increment`
  ...
  for (const [name, fields] of Object.entries(models)) {
    ...
    const item = {}
    for (const [fName, fType] of Object.entries(fields)) {
      item[fName] = getSmartMockValue(fName, fType)
    }
    mockData[name] = item
  }

  // 默认回滚模型 (如果没有定义模型)
  const fallbackMock = { id: '@increment', title: '@ctitle(8)', price: '@float(1, 100, 2, 2)' }
  const firstModelMock = Object.values(mockData)[0] || fallbackMock
    ...
    response: () => {
      return {
        ...
        data: Array(10).fill(null).map((_, i) => ({
          ...${JSON.stringify(firstModelMock, null, 10)}
        })),
        message: 'ok'
      }
    }
