...
## 4. 增强的视图治理 (v3.4.0 模板升级)

从 `v3.4.0` 开始，官方内置模板（如 `vue3-vant`）不再只是生成一个简单的列表容器，而是内置了完整的工业级状态管理，大幅提升了生成代码的「首屏可用性」：

- **骨架屏渲染 (Skeleton)**：在页面初次加载且数据为空时，自动展示骨架占位图，告别白屏尴尬，提升感知加载速度。
- **错误容灾 UI (Error Handling)**：内置错误捕获逻辑与 UI。当 API 请求失败时，页面会自动展示错误信息并提供「重新加载」按钮，无需开发者手动处理基础容错。
- **空态管理 (Empty State)**：当数据加载完成但结果集为空时，自动展示 Vant 标准空态图，交互更加自然。
- **搜索增强 (Search Bar Integration)**：若在 Schema 中声明了 `search_bar: true` 特性，模板将自动注入检索组件，并与 `useHook` 中的刷新逻辑无缝绑定。
- **数据追踪锚位 (Tracking Hooks)**：默认为关键交互（如重试按钮、列表项）注入 `data-track-id` 语义化标识，方便企业级埋格方案进行无感全量采集。
